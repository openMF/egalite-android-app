...
public class RepayAgendaListAdapter extends BaseAdapter implements Filterable {
	...
	public RepayAgendaListAdapter(Context context,
			...
			private void handleListItem(final int position) {
				...
				if (mDisplayedList.get(position).getAgendaStatus() != null) {
					if (mDisplayedList.get(position).getAgendaStatus()
						...
						CommonContexts.SELECTED_REPAY = (AgendaMaster) mDisplayedList
								.get(position);
						mPager.setAdapter(LoanRepayView.adapter);

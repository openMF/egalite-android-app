...
public class ClcAgendaListAdapter extends BaseAdapter implements Filterable {
	private Context mContext;
	private LayoutInflater mLayoutInflater;
	private List<AgendaMaster> mClcAgendaList;
	private List<AgendaMaster> mDisplayedList;
	private ClcAgendaListAdapter mlistAdapter;
	private ListView mListView;
	private BfsiViewPager mPager;
	private HolderFilter mHolderFilter;

	public ClcAgendaListAdapter(Context context,
			List<AgendaMaster> agendaMaster, ListView mView, BfsiViewPager mPage) {
		mContext = context;
		mLayoutInflater = LayoutInflater.from(mContext);
		mClcAgendaList = agendaMaster;
		mDisplayedList = mClcAgendaList;
		mPager = mPage;
		mListView = mView;
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = mLayoutInflater.inflate(
					R.layout.deposit_agenda_collection_listitem, null);
			holder = new ViewHolder();
			holder.name = (TextView) convertView
					.findViewById(R.id.txv_depo_agen_col_listitem_name);
			holder.acno = (TextView) convertView
					.findViewById(R.id.txv_depo_agen_col_listitem_accno);
			holder.amountDue = (TextView) convertView
					.findViewById(R.id.txv_depo_agen_col_listitem_amtdue);
			holder.rel_layout = (RelativeLayout) convertView
					.findViewById(R.id.rellay_clc_agenda_listitem);
			...
			private void handleListItem(final int position) {
				...
				if (mDisplayedList.get(position).getAgendaStatus() != null) {
					if (mDisplayedList.get(position).getAgendaStatus()
						...
						CommonContexts.SELECTED_CLC =(AgendaMaster) mDisplayedList
								.get(position);
						mPager.setAdapter(DpCollectionView.adapter);
	...
	private void applySort(Comparator<AgendaMaster> comparator) {
		...
		mlistAdapter = new ClcAgendaListAdapter(mContext, mDisplayedList,
				mListView, mPager);
	...
	private class HolderFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint == null ) {
				...
				for (AgendaMaster h : mClcAgendaList) {

...
import com.bfsi.egalite.view.loan.disbursement.DsbAgenda;
 ...
 * @author Vijay
 */
public class LoanDsbView extends BaseActivity {
	...
	class FragmentsAdapterDsb extends FragmentStatePagerAdapter {
		...
		public Fragment getItem(int position) {
			...
			case 0:
				return DsbAgenda
						.newInstance(getApplicationContext(), mPager,mPageIndicator);
			case 1:
				return DsbEntry.newInstance(mPageIndicator, mPager,CONTENT);
	...
	protected void onLeftAction() {
		...
		if (id == R.drawable.home && mPager.getCurrentItem() == 0) {
			Intent cash = new Intent(LoanDsbView.this, HomeView.class);
			startActivity(cash);
			finish();
	...
	private void handleBackPressed() {
		if (mPager.getCurrentItem() == 0) {
			...
			DsbEntry fragment = (DsbEntry) mRegisteredFragments.get(mPager
					.getCurrentItem());

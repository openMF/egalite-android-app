...
import com.bfsi.egalite.adapters.RepayAgendaListAdapter;
...
public class RepayAgenda extends Fragment implements CommandListener {
	private Logger LOG = LoggerFactory.getLogger(getClass());
	private RepayAgendaListAdapter mRepayListAdapter;
	...
	public static RepayAgenda newInstance(BfsiViewPager pager
			) {
		RepayAgenda fragment = new RepayAgenda();
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				if (s.length() >= 1 && mRepayListAdapter != null) {
					...
					if (mRepaymentList != null && mRepaymentList.size() > 0) {
						mRepayListAdapter = new RepayAgendaListAdapter(
	...
	private void agendaList() {
		...
		try {
			...
			mRepaymentList = rpyDao.repayAgenda();
		...
		mRepayListAdapter = new RepayAgendaListAdapter(mContext,

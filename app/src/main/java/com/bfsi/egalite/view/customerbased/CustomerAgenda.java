...
import com.bfsi.egalite.adapters.CustAgendaListAdapter;
...
public class CustomerAgenda extends Fragment implements CommandListener {
	...
	private List<AgendaMaster> mCustAgendaList = new ArrayList<AgendaMaster>();
	

	public static CustomerAgenda newInstance(Context context, BfsiViewPager pager,
			TabPageIndicator pageIndicatory) {
		CustomerAgenda fragment = new CustomerAgenda();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mView = inflater.inflate(R.layout.customer_agenda, null);
		...
		mSearchBox.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				if (s.length() >= 1 && mlistAdapter != null) {
					...
					if (mCustAgendaList != null && mCustAgendaList.size() > 0) {
						mlistAdapter = new CustAgendaListAdapter(mContext, mCustAgendaList,mListView,
								mPager);
	...
	private void agendaList() {
		CustomerDao dsbDao = DaoFactory.getCustomerDao();
		try {

			LOG.debug(mContext.getResources().getString(R.string.MFB00105));
			mCustAgendaList = dsbDao.customerAgenda();

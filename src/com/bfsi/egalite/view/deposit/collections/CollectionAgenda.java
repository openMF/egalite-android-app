...
import com.bfsi.egalite.adapters.ClcAgendaListAdapter;
...
public class CollectionAgenda extends Fragment implements CommandListener {
	...
	private List<AgendaMaster> mClcList;
	private ClcAgendaListAdapter mlistAdapter;

	public static CollectionAgenda newInstance(Context context,
			BfsiViewPager pager) {
		CollectionAgenda fragment = new CollectionAgenda();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mView = inflater.inflate(R.layout.deposit_agenda_collection, null);
		return mView;
	}
	...
	private void agendaList() {
		...
		try {
			...
			mClcList = clcDao.readAgendaCollections();
		...
		mlistAdapter = new ClcAgendaListAdapter(mContext, mClcList, mListView,
				mPager);
		...
		mSearchBox.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence s, int start, int before,
				...
				if (s.length() >= 1 && mlistAdapter != null) {
					...
					if (mClcList != null && mClcList.size() > 0) {
						mlistAdapter = new ClcAgendaListAdapter(mContext,
								mClcList, mListView, mPager);
						mListView.setAdapter(mlistAdapter);
					}
				}
			}

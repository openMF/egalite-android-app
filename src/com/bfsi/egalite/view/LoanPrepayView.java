...
import com.bfsi.egalite.view.loanprepay.LoanPrepayAgenda;
import com.bfsi.egalite.view.loanprepay.LoanPrepayEntry;

public class LoanPrepayView extends BaseActivity{
	...
	class FragmentsAdapter extends FragmentStatePagerAdapter {
		...
		public Fragment getItem(int position) {
			...
			switch (position) {
			case 0:
				return LoanPrepayAgenda.newInstance(mPager);
			case 1:
				return LoanPrepayEntry.newInstance(mPageIndicator,mPager);
			default:
				return LoanPrepayAgenda.newInstance(mPager);
			}
		}
	...
	protected void onLeftAction() {
		...
		if (id == R.drawable.home && mPager.getCurrentItem() == 0) {
			Intent cash = new Intent(LoanPrepayView.this, HomeView.class);
			startActivity(cash);
			finish();
	...
	private void handleBackPressed() {
			...
			{
				LoanPrepayEntry fragment = (LoanPrepayEntry)mRegisteredFragments.get(mPager.getCurrentItem());
